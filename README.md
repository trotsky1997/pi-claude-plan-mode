...
- `/claude-plan apply-fresh` - if a fresh-session approval is pending, open the new implementation session and inject the implementation prompt via the new-session setup hook
...
7. Fresh-session approval stages the handoff and prefills `/claude-plan apply-fresh` in the editor. When the user submits that command, the extension opens a new session automatically and injects the generated implementation prompt through the new-session setup hook.
...
- fresh-session handoff is split across the tool/UI boundary: approval stages the handoff, then `/claude-plan apply-fresh` creates the new session with `ctx.newSession()` and injects the implementation prompt through the new-session setup hook
